...
- Nav：页顶为全宽透明条；滚动后收缩为悬浮药丸（左右/顶留白、大圆角、纸感底 + 轻投影 + 细发丝边），从形态上消解 iOS Safari 顶栏贴边露缝的割裂感；当前区块 photon 下划线（IntersectionObserver）；底缘 royal → photon 滚动进度线（rAF 节流）；移动端展开菜单为药丸下方独立圆角面板
