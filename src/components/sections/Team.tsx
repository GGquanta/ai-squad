...
export function Team() {
  return (
    <Section id="team" index="01" eyebrow={team.eyebrow} title={team.title}>
      ...
      <Reveal delay={0.12}>
        <figure className="card relative mt-12 overflow-hidden px-6 py-8 sm:px-10 sm:py-9">
          <span
          ...
          <blockquote className="relative max-w-3xl pl-2 text-[1rem] leading-[2.05] text-[var(--color-text-secondary)] sm:pl-4 sm:text-[1.05rem]">
            {team.playful.prefix}
            {team.playful.segments.map((seg, i) => (
              <span key={seg.brands}>
                ...
                的<span className="hl-note">{seg.note}</span>
              </span>
            ))}
            。
          </blockquote>
